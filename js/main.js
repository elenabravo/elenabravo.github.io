;(function () {
  ...
  function centerInit() {
    ...
    $('.hero__content').css({
      'margin-top':
        ($(window).height() - $('.hero__content').height()) / 2 + 'px'
    })
  ...
  function initializePortfolio() {
    ...
    $(window).bind('hashchange', function () {
      ...
      if (hash.substr(0, rootLength) != root) {
        ...
        $('.work__content')
          .find('.work__content__thumbnails.active-folio')
          .removeClass('active-folio')
        $('.work__content')
          .find(
            '.ajax-section__content.active-ajax, .ajax-section__project-navigation.active-ajax, .ajax-section__project-close.active-ajax, .ajax-section__loader.active-ajax'
          )
          .removeClass('active-ajax')

        portfolioGrid
          .find('.project a[href="#!' + url + '"]')
          .parent()
          .addClass('current')
        portfolioGrid
        ...
        portfolioGrid
          .find('.project a[href="#!' + url + '"]')
          .parents('.work__content__thumbnails')
          .addClass('active-folio')
        $('.active-folio')
          .siblings('.ajax-section')
          .children(
            '.ajax-section__content, .ajax-section__project-navigation, .ajax-section__project-close, .ajax-section__loader'
          )
          .addClass('active-ajax')
        ...
        if (pageRefresh == true && hash.substr(0, rootLength) == root) {
          $('#work')
            .stop()
            .animate({ scrollTop: 280 }, 800, 'easeOutExpo', function () {
              loadProject()
            })
            ...
            .animate({ scrollTop: 280 }, 800, 'easeOutExpo', function () {
              if (content == false) {
                ...
                projectContainer.animate(
                  { opacity: 0, height: wrapperHeight },
                  function () {
                    loadProject()
                  }
                )
          ...
          $('#work')
            .stop()
            .animate({ scrollTop: scrollPosition + 'px' }, 1000, function () {
              deleteProject()
            })
        }
      }
    ...
    function loadProject() {
      ...
      if (!ajaxLoading) {
        ...
        projectContainer.load(url + ' section#ajaxpage', function (
          xhr,
          statusText,
          request
        ) {
    ...
    function showProject() {
      ...
      if (content == false) {
        wrapperHeight =
          projectContainer.children('#ajaxpage').outerHeight() + 'px'
        projectContainer.animate(
          { opacity: 1, height: wrapperHeight },
          function () {
      ...
      if (projectIndex == projectLength) {
        ...
        $(
          '.ajax-section__project-navigation .next a, .ajax-section__project-navigation .prev a'
    ...
    function deleteProject(closeURL) {
      ...
      $('#work')
        .stop()
        .animate({ scrollTop: scrollHelper + 'px' }, 600)
